    ...
    public function enqueue_assets( $hook_suffix ) {
        if( $this->hook_suffix ){
            if( $this->hook_suffix == $hook_suffix ){
                wp_enqueue_script( 'htrp-plugin-install-manager', $this->assets_url . '/js/plugins_install_manager.js', array('jquery','wp-util', 'updates'), CF7_EXTENTIONS_PL_VERSION, true );
            }
        } else {
            wp_enqueue_script( 'htrp-plugin-install-manager', $this->assets_url . '/js/plugins_install_manager.js', array('jquery','wp-util', 'updates'), CF7_EXTENTIONS_PL_VERSION, true );
        }
    ...
    public function active_install_count( $active_installs ){
        ...
        if ( $active_installs >= 1000000 ) {
            ...
            $active_installs_text     = sprintf(
                ...
                _nx( '%s+ Million', '%s+ Million', $active_installs_millions, 'Active plugin installations', 'cf7-extensions' ),
                number_format_i18n( $active_installs_millions )
            );
        } elseif ( 0 === $active_installs ) {
            $active_installs_text = _x( 'Less Than 10', 'Active plugin installations', 'cf7-extensions' );
        } else {
