<?php
namespace HTCf7Ext\Api;

use WP_REST_Controller;
use HTCf7Ext\SanitizeTrail\Sanitize_Trait;

if ( !class_exists( '\HTCf7Ext\Admin\Options_Field'  ) ) {
    require_once HTCF7EXTOPT_INCLUDES . '/classes/Admin/Options_field.php';
}
...
class Settings extends WP_REST_Controller {
    ...
    public function create_items( $request ) {
        ...
        if ( ! wp_verify_nonce( $request['settings']['verifynonce'], 'htcf7extopt_verifynonce' ) ) {
            return;
        }
        ...
        $settings_received  = ( !empty( $request['settings'] ) ? htcf7extopt_data_clean( $request['settings'] ) : '' );
