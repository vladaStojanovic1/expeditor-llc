...
if ( ! class_exists( 'UACF7_Taxonomy_Metabox' ) ) {
	class UACF7_Taxonomy_Metabox {
			...
			if ( ! empty( $taxonomy_request ) && ! empty( $this->taxonomy_fields ) ) {
				foreach ( $this->taxonomy_fields as $field ) {
					...
					if ( ! empty( $field['id'] ) ) {
						...
						$data       = $fieldClass == 'UACF7_repeater' || $fieldClass == 'UACF7_map' || $fieldClass == 'UACF7_tab' || $fieldClass == 'UACF7_color' ? serialize( $data ) : $data;
